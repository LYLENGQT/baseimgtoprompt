...
export default function ResultsSection({ prompts }: ResultsSectionProps) {
  const handleDownloadText = () => {
    const text = prompts.map((p, i) => `Variation ${i + 1}\n${p}`).join("\n\n---\n\n");
    download("fashion-prompts.txt", text, "text/plain;charset=utf-8");
  };

  const handleDownloadJson = () => {
    const json = JSON.stringify({ input: prompts.map((p) => ({ prompt: p })) }, null, 2);
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <h2 className="text-xl font-semibold text-foreground">Your Fashion Photography Prompts</h2>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={handleDownloadText}>Download as Text</Button>
